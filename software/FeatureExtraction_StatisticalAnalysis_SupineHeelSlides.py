# -*- coding: utf-8 -*-
"""Phase2_FeatureExtractionCode_StatisticalAnalysisApproach_SupineHeelSlides.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1flXSYc-8exOGsm8BSGoLCDabJ9fHOrLG
"""
...
def compute_rep_features(
    ...
    return row
         '''   vals = state_frames[col].values if len(state_frames) > 0 else np.array([np.nan]) #Extract the values of a particular column belonging to a particular state only if any frames belong to that state.

            for stat_name, stat_fn in STAT_FUNCS.items(): #Calculate the statistics for the state. Mean, Min, Max, Range and Standard Deviation
                row[f"S{s}_{label}_{stat_name}"] = ( #S2_knee_mean
                    float(stat_fn(vals)) if len(vals) > 0 else np.nan
                ) #stat_fn(vals) calculates the stats of the values that were extracted earlier for that particular state
    row["pelvic_lift_max"] = row["S1_pelvic_mean"] - np.nanmin([
        row["S2_pelvic_min"], row["S3_pelvic_min"], row["S4_pelvic_min"] #The lowest value of the pelvic lift is found and is subtracted from the baseline (which is calculated when the body is at rest or in S1). This tells us
        #the maximum amount of pelvic lift.
    ])

    return row #Returns the row of data that contains everything from the statics of each state to basic rep data like the start frame, end frame and valley, as well as basic metadata such as patient id, rep no., etc. '''
