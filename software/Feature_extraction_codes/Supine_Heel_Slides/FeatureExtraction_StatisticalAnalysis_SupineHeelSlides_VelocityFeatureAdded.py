...
def compute_angular_velocity(smooth_df: pd.DataFrame, fps: float) -> np.ndarray:
    """
    First derivative of the smoothed knee angle, in degrees/second.
    Negative during flexion (angle decreasing), positive during extension.
    Uses the SAME savgol window as position smoothing — differentiating an
    already-smoothed signal, not the raw jittery one, to avoid amplifying noise.
    """
    dt = 1.0 / fps
    velocity = savgol_filter(
        smooth_df["knee_angle"].values,
...
ANGLE_COLS   = ["knee_angle", "hip_angle", "ankle_angle",  "pelvic_gap", "heel_y", "knee_velocity"] #An array is defined that stores the 4 main angles
ANGLE_LABELS = ["knee", "hip", "ankle", "pelvic", "heel", "velocity"] #Stores the labels
...
VIS_COLS = {
    ...
    "heel_y": ["heel_vis"],
    "knee_velocity": ["hip_vis", "knee_vis", "ankle_vis"]   
...
def compute_rep_features(
    ...
    row["S2_S4_velocity_ratio"] = abs(row["S2_velocity_mean"]) / abs((row["S4_velocity_mean"] + 1e-8)) #Here we take absolute since velocity is signed. Negative during flexion since the angle is decreasing and 
    #positive during extension since the angle is increasing. Therefore to avoid negative signs we only consider the magnitude. (Velocity is calculated by taking the derivative of position found using the savgol filter.)
    #The savgol filter when assigned deriv = 0 finds the position of the center with respect to a small window using a polynomial (in our case, cubic polynomial). When we differentiate that cubic polynomial 
    #we get slope which is essentially the velocity at that center point.  savgol fits one local polynomial, and that single fit can be read out either as the position (deriv=0) or the slope (deriv=1) - both come from the same fitting operation, just evaluated differently.
...
def process_video(
    ...
    smooth_df["knee_velocity"] = compute_angular_velocity(smooth_df, fps)
