...
SMOOTH_WINDOW = 21     #Smoothing window is set to 11, must be odd so that the middle value can be found easily. Points before and after the center value help estimate the smoothed points. For noiser signals, the value should be higher.
...
#Rep detection
REP_MIN_PROMINENCE = 10          #Only bends greater than 15 degrees will be counted as valid reps. 175 degree -> 168 degree. Thats only 7 degrees (very small movement) and hence it wont be counted as a valid rep
...
def detect_reps(hip_angles: np.ndarray) -> list[dict]: #knee_angles is a numpy array that stores knee angles across video frames
    """
    ...
    boundary_peaks = [int(np.argmax(hip_angles[: valleys[0]]))]
    ...
    for i in range(len(valleys) - 1): #We are comparings pairs of valleys. Between two consecutive valleys there is one peak which marks the end of rep 1 and the start of rep 2
        window     = hip_angles[valleys[i] : valleys[i + 1]] #We extract all the knee angles between the two valleys
    ...
    last_window = hip_angles[valleys[-1] :] #valleys[-1] indicates the last element of the array. We consider all frames from the last valley to the end of the video and hence extract all knee angles between those frames.
    ...
    for i, valley in enumerate(valleys): #enumerate returns the index and the value at that index
        ...
        rep_range = hip_angles[start] - hip_angles[valley] #ROM is calculated as the difference between the angles at extension and flexion
...
#For eg: Frame 100 -> start of the rep, Frame 150 -> maximum flexion (valley), Frame 200 -> end of the rep
    ...
    """
    rep_angles   = hip_angles[start : end + 1] #Return hip angles of that rep
    ...
    for i, angle in enumerate(rep_angles): #assign states
        if angle >= thresh_high:
            states[i] = 1                            #S1 — Resting Position
        elif angle <= thresh_low:
            states[i] = 3                            #S3 — Maximum Elevation
        elif i <= valley_local:
            states[i] = 2                            #S2 — Leg Lifting Upward
        else:
            states[i] = 4                            #S4 — Leg Lowering
...
def compute_rep_features(
    ...
    row["hip_rom"] = row["S1_hip_mean"] - row["S3_hip_mean"]
    row["knee_deviation"] = row["S1_knee_mean"] - row["S3_knee_min"]
...
def process_video(
    ...
    for rep in reps: #Loops through all reps in reps
        ...
        states = assign_states(hip, start, valley, end) #Output: states = [1,1,1,2,2,3,3,4,4]. States are assigned each frame of a rep
        ...
        h_slice = hip[start : end + 1] #Extract only the knee angles for this rep
        print(
            f"  Rep {rep['rep_id']:>2} | frames {start}–{end} " #:>2 right alignment
            f"| hip {h_slice.max():.1f}°→{h_slice.min():.1f}° " #print the rep's max and min for knee angle
            f"| "
...
for rep in reps:
    print(f"Rep {rep['rep_id']}: valley at frame {rep['valley_frame']}, "
          f"hip = {hip[rep['valley_frame']]:.1f}°")

"""Diagnostic Plot (To identify if the code is properly marking reps or not, on the basis of the tracking angle)"""

def plot_rep_debug(smooth_df: pd.DataFrame, reps: list[dict], hip: np.ndarray): #Inputs to this function are smoothed angles, detected reps and a numpy array containing the knee angles
    """
    ...
    STATE_NAMES  = {1: "S1 Resting", 2: "S2 Leg Lifting",
                    3: "S3 Peak Elevation", 4: "S4 Leg Lowering"}
    ...
    for rep in reps:
        ...
        valley = rep["valley_frame"]
        states = assign_states(hip, start, valley, end)
        ...
        axes[0].text(valley, hip[valley] - 3, f"R{rep['rep_id']}",
    ...
    axes[0].set_title("Hip angle with rep states", fontsize=10) #title is provided to the graph
...
plt.plot(hip)
plt.title("Hip angle over time") #We do this to make sure how many valleys are present. Number of valleys = Number of reps
plt.xlabel("Frame")
plt.ylabel("Degrees")
plt.grid(alpha=0.3)
plt.show()